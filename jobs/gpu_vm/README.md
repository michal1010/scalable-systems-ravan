...
```bash
bash jobs/a100_vm/run_fedit.sh
bash jobs/a100_vm/run_ravan_gs.sh
bash jobs/a100_vm/run_ravan_svd.sh
```

Run the full grid:

```bash
bash jobs/a100_vm/sweep.sh
```
...
```bash
MAX_PARALLEL=2 bash jobs/a100_vm/sweep.sh
```

For an A100 with enough memory, you can try:

```bash
MAX_PARALLEL=3 bash jobs/a100_vm/sweep.sh
```

If GPU memory becomes tight, use:

```bash
MAX_PARALLEL=1 bash jobs/a100_vm/sweep.sh
```

Useful environment overrides:

```bash
CONDA_ENV=ravan MAX_PARALLEL=2 bash jobs/a100_vm/sweep.sh
PYTHON_BIN=/path/to/python bash jobs/a100_vm/run_fedit.sh
CUDA_VISIBLE_DEVICES=0 bash jobs/a100_vm/sweep.sh
```
