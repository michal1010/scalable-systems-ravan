"""Federated classifier-head-only baseline.
...
def run(args):
    ...
    method_label = "No-LoRA" if args.model_type == "distilbert" else "T5-No-LoRA"
    ...
    print(f"  NOTE: No adapter is used — only trainable head params are averaged\n")

    prefix = "" if args.model_type == "distilbert" else f"{args.model_type}_"
    run_name = make_run_name(f"{prefix}no_lora", args.split, args.seed)
...
def main():
    parser = argparse.ArgumentParser(description="Federated no-LoRA baseline")
